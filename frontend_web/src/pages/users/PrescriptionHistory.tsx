...
const UserPrescriptionHistory = () => {
	...
	// Fetch prescriptions data after user is set
	useEffect(() => {
		const fetchPrescriptions = async () => {
			try {
				const response = await fetch(
					`http://localhost:3000/api/v1/user/prescription/all/${id}`,
					{
						headers: {
		...
		fetchPrescriptions();
	}, []); // Depend on `user` so this runs only after `user` is set
							...
							<div>
								...
								<div className="mt-4 flex space-x-4">
									<button
										onClick={() =>
											navigate(
												`/user/prescription?id=${prescription?._id}`
											)
										}
